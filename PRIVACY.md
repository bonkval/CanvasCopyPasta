...
The extension stores only user preferences on the local browser profile: the append prompt, answer list, toolbar state and position, last copied question title, and explicitly approved Canvas origins. Clipboard content is written only after the user selects a copy button. Canvas form controls change only after the user selects **Apply answers**, and the extension never submits a quiz.

Permissions are used as follows:

- `clipboardWrite`: copies the text or organized images the user requested.
